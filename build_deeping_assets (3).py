...
def add_variant_details(kind, name, variant):
    """Give every family two authored silhouettes beyond its base template.

    These pieces intentionally use the runtime material-role names so the game
    can still recolour bicolour, elemental and Ghostly equipment correctly.
    They are geometry, not decals: they catch light and remain visible through
    a full orbit in the Three.js inspector.
    """
    salt = sum((index + 1) * ord(char) for index, char in enumerate(f"{kind}:{name}"))
    if variant == 2:
        ring_height = -.05 + (salt % 7) * .035
        ring_size = .72 + (salt % 5) * .045
        torus("VARIANT_HALO", (0, .05, ring_height), ring_size, .035, ACCENT, "ACCENT", (math.pi / 2, 0, 0))
        for index in range(3 + salt % 3):
            angle = math.tau * index / (3 + salt % 3) + (salt % 13) * .07
            radius = .78 + (index % 2) * .11
            ico("VARIANT_GEM", (math.cos(angle) * radius, .14, ring_height + math.sin(angle) * radius), (.09, .055, .14), EMISSIVE, "EMISSIVE", 2)
    elif variant == 3:
        count = 4 + salt % 4
        for index in range(count):
            side = -1 if index % 2 else 1
            z = -.75 + index * 1.5 / max(1, count - 1)
            spike = cone("VARIANT_SPINE", (side * (.56 + .06 * (index % 3)), .02, z), .13 + .02 * (index % 2), 0, .42, PRIMARY if index % 3 else ACCENT, "PRIMARY" if index % 3 else "ACCENT", 20)
            spike.rotation_euler[1] = side * (math.pi / 2 - .18)
        ico("VARIANT_CORE", (0, -.08, .12), (.23, .12, .28), EMISSIVE, "EMISSIVE", 3)
...
def main():
    for name, builder in GEAR_BUILDERS.items():
        for variant in (1, 2, 3):
            clear_scene(); builder(); add_variant_details("gear", name, variant); export_asset("gear", name if variant == 1 else f"{name}_v{variant}")
    for archetype in ENEMY_ARCHETYPES:
        for variant in (1, 2, 3):
            clear_scene(); creature(archetype); add_variant_details("enemy", archetype, variant); export_asset("enemies", archetype if variant == 1 else f"{archetype}_v{variant}")
    for form, archetype in PET_FORMS.items():
        for variant in (1, 2, 3):
            clear_scene(); creature(archetype); add_variant_details("pet", form, variant); export_asset("pets", form if variant == 1 else f"{form}_v{variant}")
