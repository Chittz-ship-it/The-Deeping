...
// DEEPING VIEWER BUILD: v1.75-fire-arcane
...
function physicalMaterial(role,profile,original,objectName='',surface='metal',textures=null) {
  ...
  const emissive=profile.infrared ? '#ff1744' : profile.fireWreathed ? '#ff5a24' : profile.arcaneCharged ? '#c084fc' : profile.ultraviolet ? '#7c3aed' : role==='emissive' ? (paletteColour || profile.emissive || profile.accent) : '#000000';
  ...
  return new THREE.MeshPhysicalMaterial({
    ...
    emissiveIntensity:profile.infrared ? (role==='emissive'?3.2:.62) : profile.fireWreathed ? (role==='emissive'?3.35:.48) : profile.arcaneCharged ? (role==='emissive'?3.1:.42) : profile.ultraviolet ? (role==='emissive'?2.8:.34) : frost ? .1 : role==='emissive' ? 2.3 : 0,
...
export default function BlenderModelViewer({ src, label, size=220, compact=false, profile:providedProfile={}, fallback=null }) {
  ...
  useEffect(()=>{
    ...
    let specimen=null;
    const liveEffects={drips:[],halos:[],flames:[],arcaneRings:[],arcaneMotes:[],iridescentMaterials:[]};
    ...
    const acceptModel=gltf=>{
      ...
      specimen.traverse(node=>{
        ...
        if(profile.iridescent)liveEffects.iridescentMaterials.push(node.material);
      });
      ...
      if(profile.toxic){
        const poisonMaterial=new THREE.MeshPhysicalMaterial({color:0x65a30d,emissive:0xa3e635,emissiveIntensity:.75,roughness:.12,clearcoat:1,transparent:true,opacity:.9,transmission:.12});
      ...
      if(profile.arcaneCharged){
        const ringRadius=Math.max(span.x,span.y)*.53;
        for(let index=0;index<3;index+=1){
          const runeMaterial=new THREE.MeshBasicMaterial({color:index===1?0xe879f9:0xa855f7,transparent:true,opacity:.58-index*.08,depthWrite:false,blending:THREE.AdditiveBlending});
        ...
        const moteMaterial=new THREE.MeshBasicMaterial({color:0xf0abfc,transparent:true,opacity:.9,depthWrite:false,blending:THREE.AdditiveBlending});
        ...
        const arcaneLight=new THREE.PointLight(0xc084fc,5.3,5);arcaneLight.position.set(mid.x,mid.y,finalBox.max.z+.9);scene.add(arcaneLight);
      }
      if(profile.infrared||profile.ultraviolet){
        const colour=profile.infrared?0xff1744:0xa855f7;
        ...
        const radiance=new THREE.PointLight(colour,profile.infrared?5.8:4.2,5);radiance.position.set(mid.x,mid.y,finalBox.max.z+1);scene.add(radiance);
      }
    ...
    const tick=()=>{
      ...
      liveEffects.iridescentMaterials.forEach((material,index)=>{material.iridescenceIOR=1.58+(Math.sin(time*1.35+index*.7)+1)*.18;});
  ...
  },[sourceKey,compact,profile.primary,profile.secondary,profile.accent,profile.emissive,profile.ghostly,profile.toughened,profile.toxic,profile.frosted,profile.fireWreathed,profile.arcaneCharged,profile.infrared,profile.iridescent,profile.ultraviolet,profile.metalness,profile.roughness,elementKey]);
