...
const BIOME_LORE = [
  ...
  "Somewhere below, an arena still remembers the roar of a crowd that has been gone for centuries. The champions who fought there have not yet noticed the silence — or have simply refused to stop.",
  "Somewhere below, a vault built to outlast empires still holds a fortune no living hand will ever spend, watched over by guardians who have long since confused hoarding with purpose.",
  "Somewhere below, an orchard grows in perfect, unbroken sorrow — every tree weeping amber tears for a harvest that never comes, tended by gardeners who no longer remember why.",
  "Somewhere below, an impossible mountain range rises into a sky that shouldn't exist this deep in the earth, its peaks screaming with the voices of everyone who ever tried to reach the top and didn't come back down.",
  "Somewhere below, a black-sand desert stretches beyond any measured horizon, its storms burying and unburying the bones of every crossing ever attempted, none of them completed.",
  "Somewhere below, a menagerie built for the amusement of a court that drowned along with it still holds its animals, and the water that filled their cages taught every one of them how to hunt something smarter than themselves.",
  "Somewhere below, a field of scorched earth crackles under a storm that has never once broken, each strike of lightning finding the same ground it struck a thousand years before, and the thousand years before that.",
  "Somewhere below, an ivory tower rises past any reasonable height, its scholars still climbing toward an answer they stopped remembering the question to generations ago.",
  "Somewhere below, an entire war fought itself to a standstill and then kept fighting anyway, its machines rusting in place but never quite powering down, waiting on orders that stopped coming centuries ago.",
  "Somewhere below, a ballroom holds a masquerade that never received its final dance, its guests still turning through the same waltz in gowns that rotted around them decades ago.",
  "Somewhere below, crystal formations taller than cathedrals resonate with a single note that has never once wavered, and everything that lingers long enough to hear it clearly eventually starts humming along.",
  "Somewhere below, a carnival's lights never went dark and its carousel never stopped turning, playing the same tinny calliope tune to an audience that stopped arriving before anyone still here can remember.",
  "Somewhere below, a prison built to make its inmates forgotten did exactly that, and forgot its wardens right along with them, leaving both to patrol and wait in cells that stopped needing guarding centuries ago.",
  "Somewhere below, a laboratory dedicated to perfecting life itself instead perfected only failure, over and over, leaving every shattered jar and twitching half-finished creation as a monument to an answer that was never found.",
  "Somewhere below, an ocean should not exist and does anyway, its coastline littered with the wrecks of ships that were never meant to sail this deep, drawn down by a song that hasn't stopped since the first hull broke apart on the rocks.",
];
...
function reducer(state, action) {
  ...
  switch (action.type) {
    ...
    case 'ATTACK': {
      ...
      let dmg = damageRoll(Math.round(totalAtk(player) * bb.atkMult * regionalDmgMult), soulRendProc ? 0 : target.def);
      let crit = false;
      let megaCrit = false;
    ...
    case 'FIRE_BOW': {
      ...
      if (action.type === 'THROW') {
        if (player.throwable && player.throwable.autoKill) {
          ...
          throwBaseAtk = (baseAtk + (player.knifeDmgBonus || 0) + (isBoss ? (player.bossDmgBonus || 0) : 0)) * regionalDmgMult;
        ...
        const dmg = damageRoll(Math.round((baseAtk + rangedBonus) * bb.atkMult * regionalDmgMult), target.def);
        target.hp = Math.max(0, target.hp - dmg);
...
export default function DungeonCrawler() {
  ...
  useEffect(() => {
    ...
    newLines.forEach(line => {
      ...
      if (line.startsWith('You strike the')) {
        sfxEngine.clank();
            ...
            BIOMES.map((biome, i) => {
              ...
              return (
                <div key={i} className={`flex items-center justify-between gap-2 px-2 py-1.5 rounded dc-panel-raised ${!next ? 'rare-glow' : ''}`}>
                  <div className="min-w-0">
                    <div className="text-xs" style={{ color: '#e7e2d0' }}>{biome.name} <span style={{ color: '#9a9788' }}>({ownedCount}/3)</span></div>
                    <div className="text-[10px]" style={{ color: '#9a9788' }}>{next ? next.desc : 'Fully mastered.'}</div>
                  </div>
                  {next ? (
          ...
          <div key={cat}>
            ...
            {cat === 'coins' && (
              !prestige?.coinsTradedIn ? (
                ...
                <p className="text-[11px]" style={{ color: '#9a9788' }}>🕳️ Secret Tunnels may find you. Keep collecting.</p>
              )
            )}
            {cat === 'stamps' && (
              !prestige?.readyOrNotTreeUnlocked ? (
                ...
                <p className="text-[11px]" style={{ color: '#9a9788' }}>🎖️ Grizzled Veteran unlocked. Keep collecting.</p>
              )
            )}
            {cat === 'figures' && (
              !prestige?.betterMerchantTreeUnlocked ? (
                ...
                <p className="text-[11px]" style={{ color: '#9a9788' }}>📚 Book Smarts unlocked. Keep collecting.</p>
              )
            )}
          </div>
        );
