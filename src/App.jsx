...
// Pluralizes the final word of an enemy name (handles the common irregular
// English plurals — wolf/wolves, etc — rather than a naive "+s", since a
// few enemy names would otherwise come out wrong, e.g. "Feral Wolf" -> the
// grammatically incorrect "Feral Wolfs").
const IRREGULAR_PLURALS = {
  wolf: 'wolves', knife: 'knives', elf: 'elves', thief: 'thieves', life: 'lives',
  leaf: 'leaves', loaf: 'loaves', wife: 'wives', shelf: 'shelves', calf: 'calves',
  half: 'halves', scarf: 'scarves', hoof: 'hooves', goose: 'geese', mouse: 'mice',
  foot: 'feet', tooth: 'teeth', man: 'men', woman: 'women', child: 'children',
...
function pluralizeWord(word) {
  ...
  if (IRREGULAR_PLURALS[lower]) {
    ...
    return word[0] === word[0].toUpperCase() ? plural[0].toUpperCase() + plural.slice(1) : plural;
  }
...
function pluralizeEnemyName(name) {
...
// Procedural combat-room flavour text, built from what's actually in the
// room (enemy count, whether they're all the same type, and the current
// biome) rather than a fixed pool — gives every encounter a slightly
// different line without needing an external API call.
const COMBAT_NARRATION_SOLO = [
  (name, biomeName) => `A lone ${name} blocks the way ahead, deep in ${biomeName}.`,
  (name, biomeName) => `A ${name} steps out of the dark to meet you, here in ${biomeName}.`,
  (name, biomeName) => `Something stirs ahead — a ${name}, watching from the gloom of ${biomeName}.`,
  (name, biomeName) => `A ${name} bars your path forward through ${biomeName}.`,
  (name, biomeName) => `You are not alone. A ${name} has already noticed you, somewhere in ${biomeName}.`,
];
const COMBAT_NARRATION_PAIR_SAME = [
  (namePlural, biomeName) => `Two ${namePlural} converge on you at once, deep in ${biomeName}.`,
  (namePlural, biomeName) => `A pair of ${namePlural} moves to cut off your retreat, here in ${biomeName}.`,
  (namePlural, biomeName) => `Two ${namePlural} block the passage ahead through ${biomeName}.`,
];
const COMBAT_NARRATION_PAIR_MIXED = [
  (name1, name2, biomeName) => `A ${name1} and a ${name2} square off against you, deep in ${biomeName}.`,
  (name1, name2, biomeName) => `A ${name1} stands beside a ${name2}, both watching you approach through ${biomeName}.`,
  (name1, name2, biomeName) => `Two very different threats — a ${name1} and a ${name2} — block your way through ${biomeName}.`,
];
const COMBAT_NARRATION_GROUP_SAME = [
  (namePlural, biomeName) => `Three ${namePlural} surround you at once, deep in ${biomeName}.`,
  (namePlural, biomeName) => `A trio of ${namePlural} closes in from every side, here in ${biomeName}.`,
];
const COMBAT_NARRATION_GROUP_MIXED = [
  (namesList, biomeName) => `A ragtag band — ${namesList} — moves to intercept you, deep in ${biomeName}.`,
  (namesList, biomeName) => `Three shapes emerge from the dark of ${biomeName}: ${namesList}.`,
];
...
function generateRoomNarration(room, biomeIndex) {
  ...
  const uniqueNames = [...new Set(enemies.map(e => e.name))];
  if (enemies.length === 1) {
    return pickRandom(COMBAT_NARRATION_SOLO)(enemies[0].name, biome.name);
  }
  if (enemies.length === 2) {
    if (uniqueNames.length === 1) {
      return pickRandom(COMBAT_NARRATION_PAIR_SAME)(pluralizeEnemyName(uniqueNames[0]), biome.name);
    }
    return pickRandom(COMBAT_NARRATION_PAIR_MIXED)(uniqueNames[0], uniqueNames[1], biome.name);
  }
  // 3 enemies
  if (uniqueNames.length === 1) {
    return pickRandom(COMBAT_NARRATION_GROUP_SAME)(pluralizeEnemyName(uniqueNames[0]), biome.name);
  }
  return pickRandom(COMBAT_NARRATION_GROUP_MIXED)(uniqueNames.join(', '), biome.name);
}
