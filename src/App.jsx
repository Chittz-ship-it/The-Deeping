  ...
  switch (action.type) {
    ...
    case 'SPIN_WHEEL': {
      ...
      if (reward.type === 'gold') {
        ...
        msg = `The wheel stops on Gold — you win ${reward.amount}g!`;
...
function PrestigePanel({ prestige, onUnlock, onUnlockTrick, onUnlockReadyOrNotTier, onUnlockBetterMerchantTier, onUnlockCoins2Tier, onUnlockCoins3Tier, onUnlockMadgodTier, onUnlockPhysicianTier, onUnlockAtlas, onNewRun }) {
...
const BODY_SLOT_LABELS = {
  weapon: 'Weapon', armor: 'Armor', chestpiece: 'Chestpiece', greaves: 'Greaves', footwear: 'Footwear',
  ring1: 'Ring I', ring2: 'Ring II', ring3: 'Ring III', earring: 'Earring I', earring2: 'Earring II',
};

const BODY_SLOT_LAYOUT = [
  { key: 'earring',    top: '7%',  left: '38%' },
  { key: 'earring2',   top: '7%',  left: '62%' },
  { key: 'armor',      top: '33%', left: '50%' },
  { key: 'chestpiece', top: '47%', left: '50%' },
  { key: 'weapon',     top: '40%', left: '84%' },
  { key: 'ring1',      top: '34%', left: '16%' },
  { key: 'ring2',      top: '44%', left: '16%' },
  { key: 'ring3',      top: '54%', left: '16%' },
  { key: 'greaves',    top: '70%', left: '50%' },
  { key: 'footwear',   top: '92%', left: '50%' },
];
...
function KnightSilhouette() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
      {/* head */}
      <div style={shape({ top: '7%', left: '50%', width: 46, height: 46, borderRadius: '50%' })} />
      {/* neck */}
      <div style={shape({ top: '16%', left: '50%', width: 16, height: 10, borderRadius: 3 })} />
      {/* torso */}
      <div style={shape({ top: '40%', left: '50%', width: 92, height: 108, borderRadius: 18 })} />
      {/* arms */}
      <div style={shape({ top: '39%', left: '22%', width: 20, height: 92, borderRadius: 10 })} />
      <div style={shape({ top: '39%', left: '78%', width: 20, height: 92, borderRadius: 10 })} />
      {/* legs */}
      <div style={shape({ top: '76%', left: '39%', width: 28, height: 96, borderRadius: 10 })} />
      <div style={shape({ top: '76%', left: '61%', width: 28, height: 96, borderRadius: 10 })} />
    </div>
  );
}
...
function AnatomicalBody({ player, onExamine }) {
  ...
  return (
    <div style={{ position: 'relative', height: 300, marginBottom: 4 }}>
      <KnightSilhouette />
      {slots.map(s => (
        <div key={s.key} style={{ position: 'absolute', top: s.top, left: s.left, transform: 'translate(-50%, -50%)' }}>
          ...
          <div key={cat}>
            ...
            {cat === 'coins' && (
              !prestige?.coinsTradedIn ? (
                ...
                <p className="text-[11px]" style={{ color: '#9a9788' }}>📚 The Grand Library is open to you. Keep collecting.</p>
              )
            )}
            {cat === 'stamps' && (
              !prestige?.readyOrNotTreeUnlocked ? (
                ...
                <p className="text-[11px]" style={{ color: '#9a9788' }}>🗝️ The Relic Room is open to you. Keep collecting.</p>
              )
            )}
            {cat === 'figures' && (
              !prestige?.betterMerchantTreeUnlocked ? (
                ...
                <p className="text-[11px]" style={{ color: '#9a9788' }}>💎 The Deluxe Merchant may find you. Keep collecting.</p>
              )
            )}
          </div>
        );
