...
const COINS10_TREE = [
  { id: 'perfected_finds', name: 'Perfected Finds', desc: 'Unlocks five independent 1% item mutations: Dirty weapons, Toughened armour, Sharpened weapons, Enchanted rings/earrings, and ROYGBIV colour variants. Mutations can stack.', cost: 100 },
];
...
function applyCoinMutations(item, player) {
  if (!item || !player?.perfectedFindsUnlocked || !COIN_MUTATION_TYPES.has(item.type)) return item;
  let out = { ...item };
  const notes = [];
  if (out.type === 'weapon' && out.rarity !== 'mythic' && Math.random() < 0.01) {
    out.dirty = true;
    out.name = `Dirty ${out.name}`;
    notes.push('dirty');
  }
  if (COIN_ARMOR_TYPES.has(out.type) && Math.random() < 0.01) {
    out.toughened = true;
    out.name = `Toughened ${out.name}`;
    out = doubleItemStats(out);
    notes.push('toughened');
  }
  if (out.type === 'weapon' && Math.random() < 0.01) {
    out.sharpened = true;
    out.name = `Sharpened ${out.name}`;
    out = doubleItemStats(out);
    notes.push('sharpened');
  }
  if (COIN_ACCESSORY_TYPES.has(out.type) && Math.random() < 0.01) {
    out.enchanted = true;
    out.name = `Enchanted ${out.name}`;
    out = doubleItemStats(out);
  ...
  if (Math.random() < 0.01) {
    ...
    out.name = `${colour.name} ${out.name}`;
    out = doubleItemStats(out);
    notes.push(colour.name.toLowerCase());
  }
  if (notes.length) out.coinMutations = notes;
  return out;
}

function applyCoinMutationsToItems(items, player) {
  return player?.perfectedFindsUnlocked ? items.map(it => applyCoinMutations(it, player)) : items;
}
...
function cleanDirtyWeapon(item) {
  ...
  out.cleaned = true;
  out.name = rebuildMutationName(out, true);
  return out;
}
...
function applyPrestigeToPlayer(player, prestige) {
  ...
  p.perfectedFindsUnlocked = !!prestige.perfectedFindsUnlocked;
...
function reducer(state, action) {
  ...
  switch (action.type) {
    ...
    case 'UNLOCK_PERFECTED_FINDS': {
      if (!state.prestige.coins10TreeUnlocked || state.prestige.perfectedFindsUnlocked) return state;
      const node = COINS10_TREE[0];
      if (state.prestige.souls < node.cost) return state;
      const prestige = { ...state.prestige, souls: state.prestige.souls - node.cost, perfectedFindsUnlocked: true };
      return { ...state, prestige, log: [...state.log, '🪙 Perfected Finds purchased. Five independent 1% mutations are now possible — and they can stack.'].slice(-50) };
    }
    ...
    <div className="dc-panel rounded p-3 mb-1">
      ...
      {prestige.coins10TreeUnlocked && (
        <div className="mt-3"><Section title="Perfected Finds">{COINS10_TREE.map(node => {
          const owned = prestige.perfectedFindsUnlocked; const canAfford = prestige.souls >= node.cost;
          return <div key={node.id} className={`flex items-center justify-between gap-2 px-2 py-1.5 rounded dc-panel-raised ${owned ? 'rare-glow' : ''}`}><div className="min-w-0"><div className="text-xs" style={{ color: owned ? '#c9a4f7' : '#e7e2d0' }}>{node.name}</div><div className="text-[10px]" style={{ color: '#9a9788' }}>{node.desc}</div></div>{owned ? <span className="text-[10px] dc-rare">OWNED</span> : <SmallBtn variant="primary" disabled={!canAfford} onClick={() => dispatch({ type: 'UNLOCK_PERFECTED_FINDS' })}>{node.cost} 👻</SmallBtn>}</div>;
        })}</Section></div>
      )}
          ...
          <div key={cat}>
            ...
            {cat === 'coins' && (
              !prestige?.coinsTradedIn ? (
                ...
                <p className="text-[11px]" style={{ color: '#9a9788' }}>🪙 Perfected Finds unlocked. Keep collecting.</p>
              )
            )}
            {cat === 'stamps' && (
              !prestige?.readyOrNotTreeUnlocked ? (
