...
/* ---------------------------------------------------------
   ...
   flat each. Requires the Abyssal Archive; each tier adds 0.1% chance
...
function generateRoom(depth, player) {
  ...
  // 1.28 — rare Abyssal Anomaly. It starts appearing after the player has
  // reached the deeper game, and competes with the ordinary treasure/relic
  // rolls rather than replacing guaranteed rooms.
...
function reducer(state, action) {
  ...
  switch (action.type) {
    ...
    case 'UNLOCK_FIGURES10_TREE': {
      ...
      if (!state.prestige.abyssalArchiveUnlocked) return state;
      const owned = state.player.collectibles.figures || [];
      if (owned.length < 50) return state;
      const player = { ...state.player, figures10TreeUnlocked: true, collectibles: { ...state.player.collectibles, figures: [] } };
      const prestige = { ...state.prestige, figures10TreeUnlocked: true };
      const log = [...state.log, '🜁 You trade away your tenth figure collection — Abyssal Breach unlocks, and five new figures enter the cabinet.'];
      return { ...state, player, prestige, log: log.slice(-50) };
    }
    ...
    case 'UNLOCK_CARDS8_TREE': {
      ...
      const log = [...state.log, '👻 You trade away your eighth card collection — Soul Hunter unlocks, and five new cards enter the cabinet.'];
      return { ...state, player, prestige, log: log.slice(-50) };
    }
        ...
        return (
          <div key={cat}>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <span className="text-[11px] dc-mono" style={{ color: '#9a9788' }}>{owned.length}/55</span>
            </div>
            <div className="grid grid-cols-10 gap-1 mb-1.5">
              {Array.from({ length: 55 }).map((_, i) => {
                const has = owned.includes(i);
                const slotUnlocked = i < 50;
                const isSelected = selected && selected.category === cat && selected.index === i;
                const title = has ? getCollectibleName(player, cat, i) : slotUnlocked ? '???' : 'LOCKED';
                return (
                  <button
                    key={i}
                    title={title}
                    ...
                    style={{
                      background: has ? (isSelected ? '#33363f' : '#262936') : slotUnlocked ? '#11121a' : '#0d0e14',
                      color: has ? '#e8c468' : slotUnlocked ? '#3a3e4a' : '#55515f',
                      border: isSelected ? '1px solid #e8a23d' : has ? '1px solid #3a3e4a' : '1px solid #22242c',
                      cursor: has ? 'pointer' : 'default',
                    }}
                  >
                    {has ? meta.icon : slotUnlocked ? '·' : '🔒'}
                  </button>
                );
              })}
            </div>
            ...
            {cat === 'figures' && (
              !prestige?.betterMerchantTreeUnlocked ? (
                ...
                <div className="dc-panel-raised rounded px-2 py-1.5 flex items-center justify-between gap-2">
                  <span className="text-[11px]" style={{ color: '#9a9788' }}>Trade a tenth 50 figures to unlock Abyssal Breach and five new Figures.</span>
                  <SmallBtn variant="primary" disabled={!isComplete || !prestige?.abyssalArchiveUnlocked} onClick={() => dispatch({ type: 'UNLOCK_FIGURES10_TREE' })}>Trade In</SmallBtn>
                </div>
              ) : !prestige?.figures11TreeUnlocked ? (
                <div className="dc-panel-raised rounded px-2 py-1.5 flex items-center justify-between gap-2">
