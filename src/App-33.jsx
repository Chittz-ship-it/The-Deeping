...
function rollLoot(enemy, player) {
  ...
  const rareGearPool = [...RARE_WEAPONS, ...RARE_ARMORS, ...RARE_RINGS, ...RARE_EARRINGS, ...(hasTrinket ? RARE_TRINKETS : [])];
  ...
  if (enemy.rarity === 'legendary') {
    ...
    if (Math.random() < 0.15 + lb * 0.3) items.push(makeScaledItemInstance(pickRandom([...COMMON_RINGS, ...COMMON_EARRINGS, ...(hasTrinket ? COMMON_TRINKETS : [])]), enemy.depth));
    ...
    // Guaranteed one piece of rare gear (weapon/armor/ring/earring/trinket, plus chest/greaves/footwear if unlocked).
...
function freshPlayer(prestige, name) {
  ...
  const base = {
    ...
    chestpiecesBag: [], greavesBag: [], footwearBag: [], headgearBag: [], trinketsBag: [],
    abilities: [],
...
function reducer(state, action) {
  ...
  switch (action.type) {
    ...
    case 'ATTACK': {
      ...
      let dmg = damageRoll(Math.round(totalAtk(player) * bb.atkMult), soulRendProc ? 0 : target.def);
      let crit = false;
      let megaCrit = false;
    ...
    case 'FIRE_BOW': {
      ...
      if (action.type === 'THROW') {
        ...
        const dmg = damageRoll(Math.round((baseAtk + rangedBonus) * bb.atkMult), target.def);
        target.hp = Math.max(0, target.hp - dmg);
    ...
    case 'SELL': {
      ...
      const bagKeyMap = {
        ...
        chestpiece: 'chestpiecesBag', greaves: 'greavesBag', footwear: 'footwearBag', headgear: 'headgearBag', trinket: 'trinketsBag',
      };
    ...
    case 'EQUIP': {
      ...
      } else if (bag === 'chestpiece' || bag === 'greaves' || bag === 'footwear' || bag === 'headgear' || bag === 'trinket') {
        const bagKeyMap = { chestpiece: 'chestpiecesBag', greaves: 'greavesBag', footwear: 'footwearBag', headgear: 'headgearBag', trinket: 'trinketsBag' };
        const emptyIdMap = { chestpiece: 'no_chest', greaves: 'no_greaves', footwear: 'no_footwear', headgear: 'no_headgear', trinket: 'no_trinket' };
        const bagKey = bagKeyMap[bag];
...
function PrestigePanel({ prestige, onUnlock, onUnlockTrick, onUnlockReadyOrNotTier, onUnlockBetterMerchantTier, onUnlockCoins2Tier, onUnlockCoins3Tier, onUnlockMadgodTier, onUnlockPhysicianTier, onUnlockAtlas, onUnlockWealthTier, onUnlockHealthTier, onUnlockHeavilyArmedTier, onUnlockWellReadTier, onUnlockBookSmartsTier, onUnlockRangedMasteryTier, onUnlockStamps6Tier, onNewRun }) {
