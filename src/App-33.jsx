...
// Per-biome ambient backgrounds — CSS gradients rather than image assets, so
// they stay lightweight and match the game's existing emoji/CSS visual
// language. Applied to the root container behind everything else; subtle and
// dark so the existing dc-panel surfaces stay legible on top of them.
const BIOME_BACKGROUNDS = [
  'radial-gradient(ellipse 120% 70% at 50% 0%, #1c2620 0%, #14151b 65%)',   // 0 The Caverns — damp mossy grey-green
  'radial-gradient(ellipse 120% 70% at 50% 0%, #212a1a 0%, #14151b 65%)',   // 1 The Blighted Swamp — murky bog green
  'radial-gradient(ellipse 120% 70% at 50% 0%, #331d10 0%, #14151b 65%)',   // 2 The Molten Forge — banked-ember orange
  'radial-gradient(ellipse 120% 70% at 50% 0%, #142530 0%, #14151b 65%)',   // 3 The Frozen Wastes — icy blue
  'radial-gradient(ellipse 120% 70% at 50% 0%, #0f2830 0%, #14151b 65%)',   // 4 The Sunken Ruins — drowned teal
  'radial-gradient(ellipse 120% 70% at 50% 0%, #1e1836 0%, #14151b 65%)',   // 5 The Astral Rift — deep violet
  'radial-gradient(ellipse 120% 70% at 50% 0%, #2a2015 0%, #14151b 65%)',   // 6 The Bonewoven Reliquary — candlelit amber-bone
  'radial-gradient(ellipse 120% 70% at 50% 0%, #191c33 0%, #14151b 65%)',   // 7 The Shattered Meridian — mirror-blue violet
  'radial-gradient(ellipse 120% 70% at 50% 0%, #182410 0%, #14151b 65%)',   // 8 The Verdant Tomb — overgrown jungle green
  'radial-gradient(ellipse 120% 70% at 50% 0%, #0b0b0e 0%, #08090b 65%)',   // 9 The Obsidian Maw — near-black void
  'radial-gradient(ellipse 120% 70% at 50% 0%, #1e1622 0%, #14151b 65%)',   // 10 The Hollow Choir — dim violet-grey
  'radial-gradient(ellipse 120% 70% at 50% 0%, #2b1e0e 0%, #14151b 65%)',   // 11 The Clockwork Abyss — brass/bronze
];
...
function bestiaryTierFor(player, baseId) {
  ...
  const thresholds = player.cards3Unlocked ? BESTIARY_THRESHOLDS_EXTENDED2
  ...
  enemies.forEach(e => {
    if (e.hp <= 0 && !e.lootGranted) {
      ...
      if (p.bestiaryUnlocked) {
        const milestoneThresholds = p.cards3Unlocked ? BESTIARY_THRESHOLDS_EXTENDED2
...
function reducer(state, action) {
  ...
  switch (action.type) {
    ...
    case 'USE_TUNNEL': {
      ...
      if (state.prestige.souls < TUNNEL_COST) return state;
      const newDepth = state.depth + 20;
      const oldBiome = currentBiome(state.depth);
      const newBiome = currentBiome(newDepth);
      let prestige = { ...state.prestige, souls: state.prestige.souls - TUNNEL_COST };
      let extraLog = [];
    ...
    case 'READ_BOOK': {
      ...
      let log = [...state.log];
      const doubled = (player.wellReadChance || 0) > 0 && Math.random() * 100 < player.wellReadChance;
      ...
      if (book.rarity === 'common') {
        ...
        log.push(`You study the ${book.name}. ${describeEffect(eff)}${doubled ? ' 📖 Well-Read! The lesson sinks in twice.' : ''}`);
      } else {
        if (player.abilities.includes(book.ability)) {
          const refund = 25 * times;
          player.gold += refund;
          log.push(`You already know this technique. The ${book.name} crumbles to dust, leaving ${refund} gold behind.${doubled ? ' 📖 Well-Read!' : ''}`);
        } else {
          player.abilities = [...player.abilities, book.ability];
          log.push(`✦ You master the ${book.name}! New ability: ${ABILITY_INFO[book.ability].name}.${doubled ? ' 📖 Well-Read! The lesson echoes, though the ability need only be learned once.' : ''}`);
        }
      }
      return { ...state, player, log: log.slice(-50) };
    }
...
function TunnelPanel({ room, depth, prestige, onUse, onDescend }) {
  const targetDepth = depth + 20;
  const targetBiome = BIOMES[currentBiome(targetDepth)];
  const canAfford = prestige.souls >= TUNNEL_COST;
  return (
    <div className="dc-panel rounded p-3 mb-1 text-center">
      ...
      {!room.used ? (
        <>
          ...
          >
            🕳️ Skip to Depth {targetDepth} ({TUNNEL_COST} 👻)
          </button>
          <div>
...
function PrestigePanel({ prestige, onUnlock, onUnlockTrick, onUnlockReadyOrNotTier, onUnlockBetterMerchantTier, onUnlockCoins2Tier, onUnlockCoins3Tier, onUnlockMadgodTier, onUnlockPhysicianTier, onUnlockAtlas, onUnlockWealthTier, onUnlockHealthTier, onUnlockHeavilyArmedTier, onUnlockWellReadTier, onNewRun }) {
          ...
          <div key={cat}>
            ...
            {cat === 'stamps' && (
              !prestige?.readyOrNotTreeUnlocked ? (
                ...
                <p className="text-[11px]" style={{ color: '#9a9788' }}>📜 Skill tiers XI-XV unlocked. Keep collecting.</p>
              )
            )}
            {cat === 'figures' && (
              !prestige?.betterMerchantTreeUnlocked ? (
                ...
                <p className="text-[11px]" style={{ color: '#9a9788' }}>📖 Well-Read unlocked. Keep collecting.</p>
              )
            )}
          </div>
        );
      ...
      {known.length === 0 ? (
        ...
        <div className="dc-panel-raised rounded p-3">
          ...
          <div className="mt-2 flex flex-wrap gap-2.5">
            {(player.cards3Unlocked ? BESTIARY_THRESHOLDS_EXTENDED2 : player.cards2Unlocked ? BESTIARY_THRESHOLDS_EXTENDED : BESTIARY_THRESHOLDS_BASE).map(t => {
        ...
        <div className="space-y-1">
          {(() => {
            const maxTier = (player.cards3Unlocked ? BESTIARY_THRESHOLDS_EXTENDED2 : player.cards2Unlocked ? BESTIARY_THRESHOLDS_EXTENDED : BESTIARY_THRESHOLDS_BASE).length;
            return known.map(e => {
