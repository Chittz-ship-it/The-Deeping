...
// v1.31 — Abyssal Anomalies now scale from Figure prestige; Soul Hunter and new collectible sets added.
const GAME_VERSION = '1.42';
...
const GEAR_RARITY_FILTER = {
  ...
  uncommon:  'hue-rotate(95deg) saturate(1.3) brightness(1.0)',
  ...
  fabled:    'saturate(2.4) brightness(1.3) hue-rotate(275deg)',
  iconic:    'hue-rotate(255deg) saturate(1.9) brightness(1.15)',
};
...
function getMutationVisual(item) {
  const filters = [];
  const colors = Array.isArray(item?.rainbowColors) ? item.rainbowColors : (item?.rainbowColor ? [item.rainbowColor] : []);
  colors.forEach(c => { if (RAINBOW_FILTERS[c]) filters.push(RAINBOW_FILTERS[c]); });
  if (item?.envenomed) filters.push('sepia(1) saturate(6) hue-rotate(55deg) brightness(0.9)');
  if (item?.ghostly) filters.push('brightness(1.2) saturate(0.7)');
  return { filter: filters.length ? filters.join(' ') : 'none', opacity: item?.ghostly ? 0.55 : 1 };
}

function mutationBadge(item) {
  if (item?.rainbowColors?.length > 1) return '🌈';
  if (item?.rainbowIcon) return item.rainbowIcon;
  if (item?.envenomed) return '☠️';
  if (item?.ghostly) return '👻';
  if (item?.cursedMutation) return '☠️';
  if (item?.sharpened) return '⚔️';
  if (item?.toughened) return '🛡️';
...
function biomeIntroLine(biomeIndex, visitIndex) {
  const lines = BIOME_INTRO_LINES[biomeIndex] || BIOME_INTRO_LINES[0];
  const idx = ((visitIndex % lines.length) + lines.length) % lines.length;
  return lines[idx];
}
...
const COINS10_TREE = [
  ...
  { id: 'perfected_finds4', tier: 4, name: 'Perfected Finds IV', desc: '1% chance for a Ring or Earring to be Enchanted, doubling its stat bonuses.', cost: 20 },
...
const COIN_MUTATION_TYPES = new Set(['weapon', 'armor', 'chestpiece', 'greaves', 'footwear', 'headgear', 'trinket', 'necklace', 'ring', 'earring']);
const COIN_ARMOR_TYPES = new Set(['armor', 'chestpiece', 'greaves', 'footwear', 'headgear']);
const COIN_ACCESSORY_TYPES = new Set(['ring', 'earring']);
const ROYGBIV = [
...
function rebuildMutationName(item) {
  let base = String(item.name || '').replace(/^(Dirty|Toughened|Sharpened|Enchanted|Envenomed|Ghostly|Cursed|Red|Orange|Yellow|Green|Blue|Indigo|Violet|Cleaned|Perfected)\s+/g, '');
  const prefixes = [];
  if (item.dirty) prefixes.push('Dirty');
  if (item.cleaned) prefixes.push('Cleaned');
  if (item.rainbowColor) prefixes.push(item.rainbowColor);
...
function applyCoinMutations(item, player) {
  if (!item || item.rarity === 'iconic' || !COIN_MUTATION_TYPES.has(item.type)) return item;
  const tier = Number(player?.perfectedFindsTier || 0);
  const further = !!player?.furtherMutationsUnlocked;
  if (!tier && !further) return item;
  let out = { ...item };
  const notes = [];
  ...
  if (tier >= 1 && out.type === 'weapon' && Math.random() < 0.01) {
    out.dirty = true;
    notes.push('dirty');
  }
  if (tier >= 2 && COIN_ARMOR_TYPES.has(out.type) && Math.random() < 0.01) {
    out.toughened = true;
    out = doubleItemStats(out);
    notes.push('toughened');
  }
  if (tier >= 3 && out.type === 'weapon' && Math.random() < 0.01) {
    out.sharpened = true;
    out = doubleItemStats(out);
    notes.push('sharpened');
  }
  if (tier >= 4 && COIN_ACCESSORY_TYPES.has(out.type) && Math.random() < 0.01) {
    out.enchanted = true;
    out = doubleItemStats(out);
    notes.push('enchanted');
  }
  if (tier >= 5 && Math.random() < 0.01) {
    const colour = pickRandom(ROYGBIV);
    out.rainbowColor = colour.name;
  ...
  if (further && out.type === 'weapon' && Math.random() < 0.01) {
    out.envenomed = true;
    out.damageMultiplier = (out.damageMultiplier || 1) * 1.2;
    notes.push('envenomed');
  }
  if (further && COIN_ARMOR_TYPES.has(out.type) && Math.random() < 0.01) {
    out.ghostly = true;
    out.dodge = (out.dodge || 0) + 1;
    notes.push('ghostly');
  }
  if (further && out.type === 'ring' && Math.random() < 0.01) {
...
function applyCoinMutationsToItems(items, player) {
  return (player?.perfectedFindsTier > 0 || player?.furtherMutationsUnlocked) ? items.map(it => applyCoinMutations(it, player)) : items;
}

function rarityAfterUpgrade(rarity) {
  const order = ['common', 'uncommon', 'rare', 'epic', 'legendary', 'mythic', 'fabled'];
  const i = order.indexOf(rarity || 'common');
  return order[Math.min(order.length - 1, Math.max(0, i + 1))];
}

function cleanDirtyWeapon(item) {
  if (!item?.dirty || item.type !== 'weapon') return item;
  const out = { ...item };
  delete out.dirty;
  const oldRarity = out.rarity || 'common';
  const newRarity = rarityAfterUpgrade(oldRarity);
...
function rollLoot(enemy, player) {
  ...
  const uncommonGearExtras = [];
  const rareGearExtras = [];
  const epicGearExtras = [];
  if (hasChest) { commonGearExtras.push(...COMMON_CHESTPIECES); uncommonGearExtras.push(...UNCOMMON_CHESTPIECES); rareGearExtras.push(...RARE_CHESTPIECES); epicGearExtras.push(...EPIC_CHESTPIECES); }
  if (hasGreaves) { commonGearExtras.push(...COMMON_GREAVES); uncommonGearExtras.push(...UNCOMMON_GREAVES); rareGearExtras.push(...RARE_GREAVES); epicGearExtras.push(...EPIC_GREAVES); }
  if (hasFootwear) { commonGearExtras.push(...COMMON_FOOTWEAR); uncommonGearExtras.push(...UNCOMMON_FOOTWEAR); rareGearExtras.push(...RARE_FOOTWEAR); epicGearExtras.push(...EPIC_FOOTWEAR); }
  if (hasHeadgear) { commonGearExtras.push(...COMMON_HEADGEAR); uncommonGearExtras.push(...UNCOMMON_HEADGEAR); rareGearExtras.push(...RARE_HEADGEAR); epicGearExtras.push(...EPIC_HEADGEAR); }
  ...
  if (enemy.rarity === 'legendary') {
    ...
    const fabledGearExtras = [];
    if (hasChest) fabledGearExtras.push(...FABLED_CHESTPIECES);
    if (hasGreaves) fabledGearExtras.push(...FABLED_GREAVES);
    if (hasFootwear) fabledGearExtras.push(...FABLED_FOOTWEAR);
    if (hasHeadgear) fabledGearExtras.push(...FABLED_HEADGEAR);
    const fabledPool = [...FABLED_WEAPONS, ...FABLED_ARMORS, ...FABLED_RINGS, ...FABLED_EARRINGS, ...fabledGearExtras];
    if (player.fabledForgeUnlocked && Math.random() < 0.02) items.push(makeScaledItemInstance(pickRandom(fabledPool), enemy.depth));
    else items.push(makeScaledItemInstance(pickRandom([...LEGENDARY_WEAPONS, ...LEGENDARY_ARMORS, ...legendaryGearExtras]), enemy.depth));
    ...
    if (Math.random() < 0.25 + lb * 0.3) {
      const pool = Math.random() < uncommonUpgradeChance
        ? [...UNCOMMON_WEAPONS, ...uncommonGearExtras.filter(g => g.type !== 'armor')]
        : [...COMMON_WEAPONS, ...commonGearExtras.filter(g => g.type !== 'armor')];
      items.push(makeScaledItemInstance(pickRandom(pool), enemy.depth));
    }
    if (Math.random() < 0.2 + lb * 0.3) {
      const pool = Math.random() < uncommonUpgradeChance
        ? [...UNCOMMON_ARMORS, ...uncommonGearExtras]
        : [...COMMON_ARMORS, ...commonGearExtras];
      items.push(makeScaledItemInstance(pickRandom(pool), enemy.depth));
    }
    if (Math.random() < 0.15 + lb * 0.3) {
      const useUncommon = Math.random() < uncommonUpgradeChance;
      const pool = useUncommon
        ? [...UNCOMMON_RINGS, ...UNCOMMON_EARRINGS, ...(hasTrinket ? UNCOMMON_TRINKETS : []), ...(hasNecklace ? UNCOMMON_NECKLACES : [])]
        : [...COMMON_RINGS, ...COMMON_EARRINGS, ...(hasTrinket ? COMMON_TRINKETS : []), ...(hasNecklace ? COMMON_NECKLACES : [])];
      items.push(makeScaledItemInstance(pickRandom(pool), enemy.depth));
    }
...
function generateMerchantStock(depth, player) {
  ...
  const uncommonPool = [
    ...UNCOMMON_WEAPONS, ...UNCOMMON_ARMORS, ...UNCOMMON_RINGS, ...UNCOMMON_EARRINGS,
  ...
  const fabledPool = [
    ...FABLED_WEAPONS, ...FABLED_ARMORS, ...FABLED_RINGS, ...FABLED_EARRINGS,
    ...(player?.bodyMods?.includes('vest') ? FABLED_CHESTPIECES : []),
    ...(player?.bodyMods?.includes('pants') ? FABLED_GREAVES : []),
    ...(player?.bodyMods?.includes('feet') ? FABLED_FOOTWEAR : []),
    ...((player?.heavilyArmedUnlocked || []).includes('headgear') ? FABLED_HEADGEAR : []),
...
function freshPlayer(prestige, name) {
  ...
  const base = {
    ...
    coins11TreeUnlocked: false,
    furtherMutationsUnlocked: false,
    fabledForgeUnlocked: false, reallyIconicUnlocked: false, colourfulIconsUnlocked: false, multicolourUnlocked: false,
    subcollectibles: { statues: [], sigils: [], curios: [], gemstones: [], reliquaries: [] },
...
function freshState(prestige, preserveCollectibles, name) {
  ...
  return {
    ...
    prestigeReady: false,
    selectedTarget: 0,
    narration: biomeIntroLine(0, newVisits - 1),
...
function totalAtk(player) {
  return player.atk
    ...
    + Math.floor(getCollectibleCount(player) * (player.hoardersAtkRate || 0));
}
function totalDef(player) {
  return player.def
    ...
    + Math.floor(getCollectibleCount(player) * (player.defensiveCollectingRate || 0));
}
...
function gearDodgeChance(player) {
  return [player.armor, player.chestpiece, player.greaves, player.footwear, player.headgear, player.weapon, player.weapon2, player.ring1, player.ring2, player.ring3, player.earring, player.earring2, player.trinket, player.necklace]
    .reduce((sum, item) => sum + (Number(item?.dodge) || 0), 0);
}

function enemyTurn(player, enemies, log) {
  let p = player;
  const lb = luckBonus(p) * 0.5;
  const baseDodge = ((p.dodgeChance || 0) + gearDodgeChance(p)) / 100;
  enemies.forEach(e => {
    if (e.hp > 0) {
  ...
  enemies.forEach(e => {
    if (e.hp <= 0 && !e.lootGranted) {
      ...
      if (Object.keys(SUBCOLLECTIBLE_META).some(k => SUBCOLLECTIBLE_META[k].unlock(p)) && Math.random() < 0.05) {
        p = applySubcollectibleFind(p, log);
      }
    }
  });
  return p;
...
function rollSubcollectible(player) {
  const available = Object.keys(SUBCOLLECTIBLE_META).filter(k => SUBCOLLECTIBLE_META[k].unlock(player));
...
function applySubcollectibleFind(player, log) {
  const hit = rollSubcollectible(player);
  if (!hit) return player;
  const list = [...(player.subcollectibles?.[hit.category] || []), hit.index];
  let p = { ...player, subcollectibles: { ...(player.subcollectibles || { statues: [], sigils: [], curios: [], gemstones: [], reliquaries: [] }), [hit.category]: list } };
...
function generateRoomNarration(room, biomeIndex) {
  ...
  if (room.type === 'merchant') {
    return BIOME_MERCHANT_LINES[biomeIndex] || BIOME_MERCHANT_LINES[0];
  }
  if (room.type === 'treasure') {
    return BIOME_TREASURE_LINES[biomeIndex] || BIOME_TREASURE_LINES[0];
  }
  if (room.type !== 'combat' || !room.enemies || room.enemies.length === 0) {
    return fallbackNarration(room);
  }
  const lines = BIOME_COMBAT_LINES[biomeIndex] || BIOME_COMBAT_LINES[0];
...
function reducer(state, action) {
  ...
  switch (action.type) {
    ...
    case 'ATTACK': {
      ...
      let dmg = damageRoll(Math.round(totalAtk(player) * weaponMutationMult * bb.atkMult * regionalDmgMult * soulwellMult), soulRendProc ? 0 : target.def);
      let crit = false;
      let megaCrit = false;
      ...
      if (tricks.includes('cleave') && Math.random() < 0.03) {
        ...
        if (otherIdx >= 0) {
          ...
          const cleaveDmg = damageRoll(Math.round(totalAtk(player) * weaponMutationMult * bb2.atkMult), other.def);
    ...
    case 'DESCEND': {
      ...
      return {
        ...
        narration: enteringNewBiome ? biomeIntroLine(biomeIdx, newVisits - 1) : generateRoomNarration(newRoom, biomeIdx),
        narrationLoading: false,
    ...
    case 'DESCEND_CHOICE': {
      ...
      if (action.payload === 'loop') {
        ...
        return {
          ...
          biomeChoicePending: false,
          log,
          narration: biomeIntroLine(biomeIdx, newVisits - 1),
          narrationLoading: false,
        };
      }
      // advance
    ...
    case 'USE_TUNNEL': {
      ...
      return {
        ...
        biomeChoicePending: false,
        log: log.slice(-50),
        narration: crossedBiome ? biomeIntroLine(newBiome, newVisits - 1) : generateRoomNarration(newRoom, newBiome),
        narrationLoading: false,
        biomeIntroId: state.biomeIntroId + 1,
      };
    }
    ...
    case 'SELL': {
      ...
      else price = item.rarity === 'iconic' ? 600 : item.rarity === 'fabled' ? 800 : item.rarity === 'mythic' ? 400 : item.rarity === 'legendary' ? 250 : item.rarity === 'epic' ? 110 : item.rarity === 'rare' ? 50 : item.rarity === 'uncommon' ? 25 : 12;
    ...
    case 'CLEAN_DIRTY_ITEM': {
      ...
      const cleaned = cleanDirtyWeapon(item);
...
function RarityTag({ rarity }) {
  const cls = rarity === 'fabled' ? 'dc-fabled' : rarity === 'iconic' ? 'dc-iconic' : rarity === 'mythic' ? 'dc-mythic' : rarity === 'legendary' ? 'dc-legendary' : rarity === 'epic' ? 'dc-epic' : rarity === 'rare' ? 'dc-rare' : rarity === 'uncommon' ? 'dc-uncommon' : 'dc-common';
  return (
...
function ItemRow({ item, actions }) {
  const Icon = item.type === 'weapon' ? Sword
    ...
    : FlaskConical;
  const colorCls = item.rarity === 'iconic' ? 'dc-iconic' : item.rarity === 'mythic' ? 'dc-mythic' : item.rarity === 'legendary' ? 'dc-legendary' : item.rarity === 'epic' ? 'dc-epic' : item.rarity === 'rare' ? 'dc-rare' : item.rarity === 'uncommon' ? 'dc-uncommon' : 'dc-common';
  return (
      ...
      <div className="flex items-center gap-2 min-w-0">
        ...
        <div className="min-w-0">
          ...
          <div className="flex gap-2 items-center flex-wrap">
            ...
            {item.rainbowColor && <span className="text-[10px]">{item.rainbowIcon} {item.rainbowColor.toUpperCase()}</span>}
...
export default function DungeonCrawler() {
  ...
  }, [state.loaded, state.showTitle, state.biomeIntroId, state.depth]);
  ...
  return (
    <div className="dc-root px-3 py-4 presentation-root" style={{ backgroundImage: BIOME_BACKGROUNDS[biomeIndex], '--biome-accent': BIOME_ACCENT[biomeIndex] }}>
      <GlobalStyle />
      <div className="ambient-vignette" style={{ background: `radial-gradient(circle at 18% 12%, ${BIOME_ACCENT[biomeIndex]}22 0%, transparent 32%), radial-gradient(circle at 85% 80%, ${BIOME_ACCENT[biomeIndex]}14 0%, transparent 38%)` }} />
      <div className="ambient-dust" aria-hidden="true" />
      {biomeReveal && (
        <div className="biome-reveal" key={biomeReveal.id}>
          <div className="biome-reveal-line" style={{ background: BIOME_ACCENT[biomeReveal.idx], boxShadow: `0 0 22px ${BIOME_ACCENT[biomeReveal.idx]}` }} />
          <div className="text-[10px] tracking-[0.35em] uppercase" style={{ color: BIOME_ACCENT[biomeReveal.idx] }}>Depth {String(state.depth).padStart(2, '0')}</div>
          <div className="dc-display text-2xl mt-1" style={{ color: '#f3ead4', textShadow: `0 0 22px ${BIOME_ACCENT[biomeReveal.idx]}88` }}>{BIOMES[biomeReveal.idx].name}</div>
          <div className="text-[10px] mt-2 max-w-xs" style={{ color: '#a7a294' }}>{BIOMES[biomeReveal.idx].desc}</div>
        </div>
      )}
      <div className="max-w-md mx-auto">
        ...
        {!gameOver && (
          <div className="dc-panel rounded mt-3">
            ...
            {tab === 'equipment' && <EquipmentPanel player={player} dispatch={dispatch} />}
            {tab === 'pack' && <PackPanel player={player} room={room} prestige={state.prestige} dispatch={dispatch} />}
...
const PRESTIGE_WEB_NODES = [
  { id:'foundations', x:50, y:50, icon:'⚔️', name:'Foundations', group:'core', links:['atlas','figures','stamps','cards','coins'] },
...
function PrestigeWebPanel(props) {
  ...
  const renderSelected = () => {
    ...
    if (n.group === 'sub') {
      const meta=SUBCOLLECTIBLE_META[selected]; const unlocked=unlockedSub(selected); const progress=subProgress(selected); const bonus=selected==='statues'?prestige.statuesDefBonus:selected==='sigils'?prestige.sigilsAtkBonus:selected==='curios'?prestige.curiosGoldBonus:selected==='gemstones'?prestige.gemstonesXpBonus:prestige.reliquariesHpBonus;
      return <div className={`dc-panel-raised rounded p-3 ${unlocked?'mythic-glow':''}`}><div className="flex items-center justify-between"><div className="dc-display text-sm">{meta.icon} {meta.label}</div><span className="dc-mono text-[11px]" style={{color:unlocked?'#5eead4':'#6b6f7a'}}>{unlocked?`${progress}/25`:'LOCKED'}</span></div><p className="text-[10px] mt-1 mb-3" style={{color:'#9a9788'}}>{meta.desc}</p>{unlocked&&<><div className="grid grid-cols-5 gap-1">{SUBCOLLECTIBLE_NAMES[selected].map((name,i)=>{const got=(prestige.subcollectibles?.[selected]||[]).includes(i); return <div key={i} title={got?name:'Undiscovered'} className="rounded text-center py-1.5" style={{background:got?'#343747':'#1a1c24',border:`1px solid ${got?'#5eead4':'#2b2e38'}`,color:got?'#e7e2d0':'#4d5160',fontSize:12}}>{got?meta.icon:'?'}</div>})}</div><div className="text-[10px] mt-2" style={{color:'#6b6f7a'}}>Permanent bonus earned so far: +{bonus||0}{selected==='curios'?'%':''} {meta.reward}.</div></>}</div>;
    }
    if (n.group === 'deep') return <div className="space-y-2"><div className="dc-panel-raised rounded p-2 flex justify-between gap-2"><div><div className="text-xs dc-display">🗺️ The Atlas</div><div className="text-[10px]" style={{color:'#9a9788'}}>Permanent biome records and Atlas gear.</div></div>{prestige.atlasUnlocked?<span className="text-[9px] dc-rare">OWNED</span>:<SmallBtn variant="primary" disabled={prestige.souls<ATLAS_COST} onClick={props.onUnlockAtlas}>{ATLAS_COST} 👻</SmallBtn>}</div><div className="dc-panel-raised rounded p-2 flex justify-between gap-2"><div><div className="text-xs dc-display">🜁 Abyssal Archive</div><div className="text-[10px]" style={{color:'#9a9788'}}>Unlocks anomaly rooms; Breach tiers remain beneath it.</div></div>{prestige.abyssalArchiveUnlocked?<span className="text-[9px] dc-rare">OPEN</span>:<SmallBtn variant="primary" disabled={!prestige.atlasUnlocked||prestige.souls<ABYSSAL_ARCHIVE_COST} onClick={props.onUnlockAbyssalArchive}>{ABYSSAL_ARCHIVE_COST} 👻</SmallBtn>}</div><div className="dc-panel-raised rounded p-2 flex justify-between gap-2"><div><div className="text-xs dc-display">🎰 The Soulwell</div><div className="text-[10px]" style={{color:'#9a9788'}}>Permanent enemy/biome micro-bonuses.</div></div>{prestige.soulwellUnlocked?<span className="text-[9px] dc-rare">OPEN</span>:<SmallBtn variant="primary" disabled={prestige.souls<SOULWELL_UNLOCK_COST} onClick={props.onUnlockSoulwell}>{SOULWELL_UNLOCK_COST} 👻</SmallBtn>}</div><div className="dc-panel-raised rounded p-2"><div className="text-xs dc-display mb-1">Abyssal Breach</div>{FIGURES10_TREE.map(node=>{const owned=(prestige.abyssalBreachUnlocked||[]).includes(node.id);return <div key={node.id} className="flex justify-between gap-2 py-1"><div><div className="text-[11px]">{node.name}</div><div className="text-[10px]" style={{color:'#9a9788'}}>{node.desc}</div></div>{owned?<span className="text-[9px] dc-rare">OWNED</span>:<SmallBtn variant="primary" disabled={!prestige.figures10TreeUnlocked||prestige.souls<node.cost} onClick={()=>props.onUnlockAbyssalBreachTier(node.id)}>{node.cost} 👻</SmallBtn>}</div>})}</div><div className="dc-panel-raised rounded p-2"><div className="text-xs dc-display mb-1">Regional Mastery</div><div className="text-[10px] mb-2" style={{color:'#9a9788'}}>Three per-biome nodes: damage, gold, then luck.</div>{!prestige.regionalMasteryTreeUnlocked?<SmallBtn variant="primary" disabled={prestige.souls<50} onClick={props.onUnlockRegionalMasteryTree}>50 👻 Unlock</SmallBtn>:<div className="space-y-1">{BIOMES.map((b,i)=><div key={i} className="flex items-center gap-1"><span className="text-[9px] w-28 truncate" style={{color:'#9a9788'}}>{b.name}</span>{REGIONAL_MASTERY_TREE.filter(n=>n.biomeIndex===i).map(node=>{const owned=(prestige.regionalMasteryUnlocked||[]).includes(node.id);const prev=node.order===0||((prestige.regionalMasteryUnlocked||[]).includes(REGIONAL_MASTERY_TREE.find(x=>x.biomeIndex===i&&x.order===node.order-1)?.id));return <SmallBtn key={node.id} variant={owned?'default':'primary'} disabled={owned||!prev||prestige.souls<node.cost} onClick={()=>props.onUnlockRegionalMasteryTier(node.id)}>{owned?'✓':`${node.cost}👻`}</SmallBtn>})}</div>)}</div>}</div><div className="dc-panel-raised rounded p-2 flex justify-between gap-2"><div><div className="text-xs dc-display">🌟 Ascension</div><div className="text-[10px]" style={{color:'#9a9788'}}>Level {prestige.ascensionLevel||0}; next milestone depth {ascensionRequiredDepth(prestige.ascensionLevel||0)}.</div></div><SmallBtn variant="primary" disabled={(player.maxDepthReached||1)<ascensionRequiredDepth(prestige.ascensionLevel||0)} onClick={props.onAscend}>Ascend</SmallBtn></div></div>;
    ...
    if (n.group === 'fabled') return <div className="dc-panel-raised rounded p-3 fabled-glow"><div className="dc-display text-base dc-fabled">✦ The Fabled Forge</div><p className="text-[11px] mt-1 mb-3" style={{color:'#9a9788'}}>Unlock a rarity beyond Mythic. Fabled gear is exceptionally rare and can still receive Perfected, coloured and mutation layers.</p>{prestige.fabledForgeUnlocked?<div className="text-[10px] dc-fabled">FABLED IS ACTIVE · 2% chance on legendary-tier gear rolls.</div>:<SmallBtn variant="primary" disabled={prestige.souls<FABLED_FORGE_COST||(prestige.bestDepthEver||1)<100} onClick={props.onUnlockFabledForge}>{FABLED_FORGE_COST} 👻 · Reach depth 100</SmallBtn>}</div>;
...
function GearExamineOverlay({ item, onClose, onClean, onSell, sellPrice }) {
  const rarityColor = item.rarity === 'fabled' ? '#f4a7ff' : item.rarity === 'iconic' ? '#8a7cf0' : item.rarity === 'mythic' ? '#5eead4' : item.rarity === 'legendary' ? '#ffd76a'
    : item.rarity === 'epic' ? '#ff9152' : item.rarity === 'rare' ? '#c9a4f7' : item.rarity === 'uncommon' ? '#6ec9e8' : '#8fae6b';
  ...
  const mutationVisual = getMutationVisual(item);
  const composedFilter = [getGearFilter(item.rarity), item.emojiFilter, mutationVisual.filter !== 'none' ? mutationVisual.filter : null].filter(Boolean).join(' ');
    ...
    >
      <div onClick={(e) => e.stopPropagation()} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14 }}>
        <div style={{ position: 'relative', display: 'inline-flex' }}>
          ...
          {mutationMark && <span style={{ position: 'absolute', right: -8, top: -8, fontSize: 22, filter: item.rainbowColor ? RAINBOW_FILTERS[item.rainbowColor] : 'none' }}>{mutationMark}</span>}
        </div>
...
function rarityColorFor(rarity) {
  return rarity === 'fabled' ? '#f4a7ff' : rarity === 'iconic' ? '#8a7cf0' : rarity === 'mythic' ? '#5eead4' : rarity === 'legendary' ? '#ffd76a'
...
function BodySlotBox({ slotKey, item, onExamine }) {
  ...
  const mutationVisual = empty ? { filter: 'none', opacity: 1 } : getMutationVisual(item);
  const filter = empty ? 'none' : [getGearFilter(item.rarity), mutationVisual.filter !== 'none' ? mutationVisual.filter : null].filter(Boolean).join(' ');
  const rarityColor = empty ? '#3a3e4a' : (perfected ? '#ffffff' : rarityColorFor(item.rarity));
  const mutationMark = empty ? null : mutationBadge(item);
  return (
    <button
    ...
    >
      <span style={{ position: 'relative', display: 'inline-flex', opacity: mutationVisual.opacity }}>
        {emoji}
        {mutationMark && <span style={{ position: 'absolute', right: -8, top: -9, fontSize: 11 }}>{mutationMark}</span>}
      </span>
    </button>
  );
}
...
function equippedSellPrice(item, player) {
  if (!item) return 0;
  let price = item.rarity === 'iconic' ? 600 : item.rarity === 'fabled' ? 800 : item.rarity === 'mythic' ? 400 : item.rarity === 'legendary' ? 250 : item.rarity === 'epic' ? 110 : item.rarity === 'rare' ? 50 : item.rarity === 'uncommon' ? 25 : 12;
      ...
      {player.accessoriesBag.length > 0 && (
        <Section title="Accessories">
          {player.accessoriesBag.map((item, idx) => {
            const colorCls = item.rarity === 'legendary' ? 'dc-legendary' : item.rarity === 'epic' ? 'dc-epic' : item.rarity === 'rare' ? 'dc-rare' : item.rarity === 'uncommon' ? 'dc-uncommon' : 'dc-common';
            return (
...
function GlobalStyle() {
  return (
    <style>{`
      ...
      .presentation-root > .max-w-md { position: relative; z-index: 1; }
      ...
      @media (prefers-reduced-motion: reduce) { .ambient-vignette,.ambient-dust,.biome-reveal,.boss-arena,.treasure-chest,.anomaly-symbol,.relic-gem,.elite-presence::after { animation:none !important; } }
    `}</style>
  );
}
