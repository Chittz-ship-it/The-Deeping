...
function buildIconicDrop(enemy) {
...
function getMutationVisual(item) {
  const filters = [];
  if (item?.rainbowColor && RAINBOW_FILTERS[item.rainbowColor]) filters.push(RAINBOW_FILTERS[item.rainbowColor]);
...
function rollLoot(enemy, player) {
  ...
  if (enemy.rarity === 'legendary') {
    ...
    if (Math.random() < ICONIC_DROP_CHANCE) items.push(makeItemInstance(buildIconicDrop(enemy)));
    gold += 100;
    greaterPotions += 2;
    potions += 2;
...
function reducer(state, action) {
  ...
  switch (action.type) {
    ...
    case 'SELL': {
      ...
      else price = item.rarity === 'iconic' ? 600 : item.rarity === 'mythic' ? 400 : item.rarity === 'legendary' ? 250 : item.rarity === 'epic' ? 110 : item.rarity === 'rare' ? 50 : item.rarity === 'uncommon' ? 25 : 12;
    ...
    case 'SELL_EQUIPPED': {
      ...
      const player = { ...state.player, gold: state.player.gold + price, [slot]: replacement };
      const log = [...state.log, `You sold your equipped ${item.name} for ${price}g.`];
      return { ...state, player, log: log.slice(-50) };
    }
...
function RarityTag({ rarity }) {
  const cls = rarity === 'iconic' ? 'dc-iconic' : rarity === 'mythic' ? 'dc-mythic' : rarity === 'legendary' ? 'dc-legendary' : rarity === 'epic' ? 'dc-epic' : rarity === 'rare' ? 'dc-rare' : rarity === 'uncommon' ? 'dc-uncommon' : 'dc-common';
  return (
...
function PrestigePanel({ prestige, player, onUnlock, onUnlockTrick, onUnlockReadyOrNotTier, onUnlockBetterMerchantTier, onUnlockCoins2Tier, onUnlockCoins3Tier, onUnlockMadgodTier, onUnlockPhysicianTier, onUnlockAtlas, onUnlockAbyssalArchive, onUnlockWealthTier, onUnlockHealthTier, onUnlockHeavilyArmedTier, onUnlockWellReadTier, onUnlockBookSmartsTier, onUnlockRangedMasteryTier, onUnlockStamps6Tier, onUnlockRegionalMasteryTree, onUnlockRegionalMasteryTier, onUnlockSoulwell, onPullSoulwell, onUnlockStamps7Tier, onUnlockAncestralMemoryTier, onUnlockFactorySealedTier, onUnlockHealthIsWealthTier, onUnlockHoardersTier, onUnlockDefensiveCollectingTier, onUnlockDimensionalGiftTier, onUnlockAbyssalBreachTier, onUnlockSoulHunterTier, onUnlockSelfStarterTier, onUnlockMeaningfulLearnerTier, onUnlockTLDR, onUnlockCleverTunnelingTier, onUnlockMerchantPerfectionTier, onUnlockBookRecycling, onUnlockPerfectedFindsTier, onUnlockFurtherMutations, onAscend, onNewRun }) {
  ...
  return (
    <div className="dc-panel rounded p-3 mb-1">
      ...
      <div className="mt-3">
        <Section title="Abyssal Archive">
          <div className={`flex items-center justify-between gap-2 px-2 py-1.5 rounded dc-panel-raised ${prestige.abyssalArchiveUnlocked ? 'mythic-glow' : ''}`}>
            <div className="min-w-0">
              <div className="text-xs" style={{ color: prestige.abyssalArchiveUnlocked ? '#c9a4f7' : '#e7e2d0' }}>🜁 The Abyssal Archive</div>
              <div className="text-[10px]" style={{ color: '#9a9788' }}>
                {prestige.abyssalArchiveUnlocked
                  ? 'Anomalies can now appear from depth 20 onward. Each one can reveal an undiscovered Coin, Card, Stamp or Figure from your existing collection.'
                  : 'Extend the Atlas into the impossible. Unlocks Abyssal Anomalies that can uncover records from your existing four collections.'}
              </div>
            </div>
            {prestige.abyssalArchiveUnlocked ? (
              <span className="text-[10px] shrink-0" style={{ color: '#c9a4f7' }}>OPEN</span>
            ) : (
              <SmallBtn
                variant="primary"
                disabled={!prestige.atlasUnlocked || prestige.souls < ABYSSAL_ARCHIVE_COST}
                onClick={onUnlockAbyssalArchive}
              >
                {ABYSSAL_ARCHIVE_COST} 👻
              </SmallBtn>
            )}
          </div>
        </Section>
      ...
      {prestige.coins8TreeUnlocked && (
        <div className="mt-3">
          <Section title="Health is Wealth">
            {COINS8_TREE.map(node => {
              const owned = (prestige.healthIsWealthUnlocked || []).includes(node.id);
              const canAfford = prestige.souls >= node.cost;
              return (
                <div key={node.id} className={`flex items-center justify-between gap-2 px-2 py-1.5 rounded dc-panel-raised ${owned ? 'rare-glow' : ''}`}>
                  <div className="min-w-0">
                  ...
                  {owned ? <span className="text-[10px] dc-rare shrink-0">OWNED</span>
                    : <SmallBtn variant="primary" disabled={!canAfford} onClick={() => onUnlockHealthIsWealthTier(node.id)}>{node.cost} 👻</SmallBtn>}
                </div>
              );
            })}
          </Section>
      ...
      {prestige.cards6TreeUnlocked && (
        <div className="mt-3">
          <Section title="Tonight on Hoarders">
            {CARDS6_TREE.map(node => {
              const owned = (prestige.hoardersUnlocked || []).includes(node.id);
              const canAfford = prestige.souls >= node.cost;
              return (
                <div key={node.id} className={`flex items-center justify-between gap-2 px-2 py-1.5 rounded dc-panel-raised ${owned ? 'rare-glow' : ''}`}>
                  <div className="min-w-0">
                  ...
                  {owned ? <span className="text-[10px] dc-rare shrink-0">OWNED</span>
                    : <SmallBtn variant="primary" disabled={!canAfford} onClick={() => onUnlockHoardersTier(node.id)}>{node.cost} 👻</SmallBtn>}
                </div>
              );
            })}
          </Section>
      ...
      {prestige.figures10TreeUnlocked && (
        <div className="mt-3">
          <Section title="Abyssal Breach">
            {FIGURES10_TREE.map(node => {
              const owned = (prestige.abyssalBreachUnlocked || []).includes(node.id);
              const canAfford = prestige.souls >= node.cost;
              return <div key={node.id} className={`flex items-center justify-between gap-2 px-2 py-1.5 rounded dc-panel-raised ${owned ? 'rare-glow' : ''}`}>
                <div className="min-w-0"><div className="text-xs" style={{ color: owned ? '#c9a4f7' : '#e7e2d0' }}>{node.name}</div><div className="text-[10px]" style={{ color: '#9a9788' }}>{node.desc}</div></div>
                {owned ? <span className="text-[10px] dc-rare shrink-0">OWNED</span> : <SmallBtn variant="primary" disabled={!canAfford} onClick={() => onUnlockAbyssalBreachTier(node.id)}>{node.cost} 👻</SmallBtn>}
              </div>;
            })}
          </Section>
        </div>
      ...
      {prestige.figures9TreeUnlocked && (
        <div className="mt-3">
          <Section title="Defensive Collecting">
            {FIGURES9_TREE.map(node => {
              const owned = (prestige.defensiveCollectingUnlocked || []).includes(node.id);
              const canAfford = prestige.souls >= node.cost;
              return (
                <div key={node.id} className={`flex items-center justify-between gap-2 px-2 py-1.5 rounded dc-panel-raised ${owned ? 'rare-glow' : ''}`}>
                  <div className="min-w-0">
                  ...
                  {owned ? <span className="text-[10px] dc-rare shrink-0">OWNED</span>
                    : <SmallBtn variant="primary" disabled={!canAfford} onClick={() => onUnlockDefensiveCollectingTier(node.id)}>{node.cost} 👻</SmallBtn>}
                </div>
              );
            })}
          </Section>
      ...
      {prestige.cards8TreeUnlocked && (
        <div className="mt-3">
          <Section title="Soul Hunter">
            {CARDS8_TREE.map(node => {
              const owned = (prestige.soulHunterUnlocked || []).includes(node.id);
              const canAfford = prestige.souls >= node.cost;
              return <div key={node.id} className={`flex items-center justify-between gap-2 px-2 py-1.5 rounded dc-panel-raised ${owned ? 'rare-glow' : ''}`}>
                <div className="min-w-0"><div className="text-xs" style={{ color: owned ? '#c9a4f7' : '#e7e2d0' }}>{node.name}</div><div className="text-[10px]" style={{ color: '#9a9788' }}>{node.desc}</div></div>
                {owned ? <span className="text-[10px] dc-rare shrink-0">OWNED</span> : <SmallBtn variant="primary" disabled={!canAfford} onClick={() => onUnlockSoulHunterTier(node.id)}>{node.cost} 👻</SmallBtn>}
              </div>;
            })}
          </Section>
        </div>
      ...
      {prestige.cards7TreeUnlocked && (
        <div className="mt-3">
          <Section title="Dimensional Gift">
            {CARDS7_TREE.map(node => {
              const owned = (prestige.dimensionalGiftUnlocked || []).includes(node.id);
              const canAfford = prestige.souls >= node.cost;
              return (
                <div key={node.id} className={`flex items-center justify-between gap-2 px-2 py-1.5 rounded dc-panel-raised ${owned ? 'rare-glow' : ''}`}>
                  <div className="min-w-0">
                  ...
                  {owned ? <span className="text-[10px] dc-rare shrink-0">OWNED</span>
                    : <SmallBtn variant="primary" disabled={!canAfford} onClick={() => onUnlockDimensionalGiftTier(node.id)}>{node.cost} 👻</SmallBtn>}
                </div>
              );
            })}
          </Section>
      ...
      {prestige.stamps8TreeUnlocked && (
        <div className="mt-3">
          <Section title="Self-Starter">
            {STAMPS8_TREE.map(node => {
              const owned = (prestige.selfStarterUnlocked || []).includes(node.id);
              const canAfford = prestige.souls >= node.cost;
              return (
                <div key={node.id} className={`flex items-center justify-between gap-2 px-2 py-1.5 rounded dc-panel-raised ${owned ? 'rare-glow' : ''}`}>
                  <div className="min-w-0">
                  ...
                  {owned ? <span className="text-[10px] dc-rare shrink-0">OWNED</span>
                    : <SmallBtn variant="primary" disabled={!canAfford} onClick={() => onUnlockSelfStarterTier(node.id)}>{node.cost} 👻</SmallBtn>}
                </div>
              );
            })}
          </Section>
      ...
      {prestige.stamps9TreeUnlocked && (
        <div className="mt-3">
          <Section title="TLDR">
            {TLDR_TREE.map(node => {
              const owned = prestige.tldrUnlocked;
              const canAfford = prestige.souls >= node.cost;
              return (
                <div key={node.id} className={`flex items-center justify-between gap-2 px-2 py-1.5 rounded dc-panel-raised ${owned ? 'rare-glow' : ''}`}>
                  <div className="min-w-0">
                  ...
                  {owned ? <span className="text-[10px] dc-rare shrink-0">OWNED</span>
                    : <SmallBtn variant="primary" disabled={!canAfford} onClick={onUnlockTLDR}>{node.cost} 👻</SmallBtn>}
                </div>
              );
            })}
          </Section>
      ...
      {prestige.coins9TreeUnlocked && (
        <div className="mt-3">
          <Section title="Meaningful Learner">
            {COINS9_TREE.map(node => {
              const owned = (prestige.meaningfulLearnerUnlocked || []).includes(node.id);
              const canAfford = prestige.souls >= node.cost;
              return (
                <div key={node.id} className={`flex items-center justify-between gap-2 px-2 py-1.5 rounded dc-panel-raised ${owned ? 'rare-glow' : ''}`}>
                  <div className="min-w-0">
                  ...
                  {owned ? <span className="text-[10px] dc-rare shrink-0">OWNED</span>
                    : <SmallBtn variant="primary" disabled={!canAfford} onClick={() => onUnlockMeaningfulLearnerTier(node.id)}>{node.cost} 👻</SmallBtn>}
                </div>
              );
            })}
          </Section>
      ...
      {prestige.stamps10TreeUnlocked && (
        <div className="mt-3"><Section title="Clever Tunneling">{STAMPS10_TREE.map(node => {
          const owned = (prestige.cleverTunnelingUnlocked || []).includes(node.id); const canAfford = prestige.souls >= node.cost;
          return <div key={node.id} className={`flex items-center justify-between gap-2 px-2 py-1.5 rounded dc-panel-raised ${owned ? 'rare-glow' : ''}`}><div className="min-w-0"><div className="text-xs" style={{ color: owned ? '#c9a4f7' : '#e7e2d0' }}>{node.name}</div><div className="text-[10px]" style={{ color: '#9a9788' }}>{node.desc}</div></div>{owned ? <span className="text-[10px] dc-rare">OWNED</span> : <SmallBtn variant="primary" disabled={!canAfford} onClick={() => onUnlockCleverTunnelingTier(node.id)}>{node.cost} 👻</SmallBtn>}</div>;
        })}</Section></div>
      )}
      {prestige.figures11TreeUnlocked && (
        <div className="mt-3"><Section title="Merchant Perfection">{FIGURES11_TREE.map(node => {
          const owned = (prestige.merchantPerfectionUnlocked || []).includes(node.id); const canAfford = prestige.souls >= node.cost;
          return <div key={node.id} className={`flex items-center justify-between gap-2 px-2 py-1.5 rounded dc-panel-raised ${owned ? 'rare-glow' : ''}`}><div className="min-w-0"><div className="text-xs" style={{ color: owned ? '#c9a4f7' : '#e7e2d0' }}>{node.name}</div><div className="text-[10px]" style={{ color: '#9a9788' }}>{node.desc}</div></div>{owned ? <span className="text-[10px] dc-rare">OWNED</span> : <SmallBtn variant="primary" disabled={!canAfford} onClick={() => onUnlockMerchantPerfectionTier(node.id)}>{node.cost} 👻</SmallBtn>}</div>;
        })}</Section></div>
      )}
      {prestige.cards9TreeUnlocked && (
        <div className="mt-3"><Section title="Book Recycling">{CARDS9_TREE.map(node => {
          const owned = prestige.bookRecyclingUnlocked; const canAfford = prestige.souls >= node.cost;
          return <div key={node.id} className={`flex items-center justify-between gap-2 px-2 py-1.5 rounded dc-panel-raised ${owned ? 'rare-glow' : ''}`}><div className="min-w-0"><div className="text-xs" style={{ color: owned ? '#c9a4f7' : '#e7e2d0' }}>{node.name}</div><div className="text-[10px]" style={{ color: '#9a9788' }}>{node.desc}</div></div>{owned ? <span className="text-[10px] dc-rare">OWNED</span> : <SmallBtn variant="primary" disabled={!canAfford} onClick={onUnlockBookRecycling}>{node.cost} 👻</SmallBtn>}</div>;
        })}</Section></div>
      )}
      {prestige.coins10TreeUnlocked && (
        <div className="mt-3"><Section title="Perfected Finds">{COINS10_TREE.map(node => {
          const unlocked = Array.isArray(prestige.perfectedFindsUnlocked) ? prestige.perfectedFindsUnlocked : (prestige.perfectedFindsUnlocked ? COINS10_TREE.map(n => n.id) : []);
          const owned = unlocked.includes(node.id); const canAfford = prestige.souls >= node.cost;
          const prerequisiteMet = node.tier === 1 || unlocked.includes(COINS10_TREE[node.tier - 2].id);
          return <div key={node.id} className={`flex items-center justify-between gap-2 px-2 py-1.5 rounded dc-panel-raised ${owned ? 'rare-glow' : ''}`}><div className="min-w-0"><div className="text-xs" style={{ color: owned ? '#c9a4f7' : '#e7e2d0' }}>{node.name}</div><div className="text-[10px]" style={{ color: '#9a9788' }}>{node.desc}</div></div>{owned ? <span className="text-[10px] dc-rare">OWNED</span> : <SmallBtn variant="primary" disabled={!canAfford || !prerequisiteMet} onClick={() => onUnlockPerfectedFindsTier(node.id)}>{node.cost} 👻</SmallBtn>}</div>;
        })}</Section></div>
      )}
      {prestige.coins11TreeUnlocked && (
        <div className="mt-3"><Section title="Further Mutations">{COINS11_TREE.map(node => {
          const owned = prestige.furtherMutationsUnlocked; const canAfford = prestige.souls >= node.cost;
          return <div key={node.id} className={`flex items-center justify-between gap-2 px-2 py-1.5 rounded dc-panel-raised ${owned ? 'rare-glow' : ''}`}><div className="min-w-0"><div className="text-xs" style={{ color: owned ? '#c9a4f7' : '#e7e2d0' }}>{node.name}</div><div className="text-[10px]" style={{ color: '#9a9788' }}>{node.desc}</div></div>{owned ? <span className="text-[10px] dc-rare">OWNED</span> : <SmallBtn variant="primary" disabled={!canAfford} onClick={onUnlockFurtherMutations}>{node.cost} 👻</SmallBtn>}</div>;
        })}</Section></div>
      )}
...
function GearExamineOverlay({ item, onClose, onClean, onSell, sellPrice }) {
  const rarityColor = item.rarity === 'iconic' ? '#8a7cf0' : item.rarity === 'mythic' ? '#5eead4' : item.rarity === 'legendary' ? '#ffd76a'
...
function rarityColorFor(rarity) {
  return rarity === 'iconic' ? '#8a7cf0' : rarity === 'mythic' ? '#5eead4' : rarity === 'legendary' ? '#ffd76a'
...
function equippedSellPrice(item, player) {
  if (!item) return 0;
  let price = item.rarity === 'iconic' ? 600 : item.rarity === 'mythic' ? 400 : item.rarity === 'legendary' ? 250 : item.rarity === 'epic' ? 110 : item.rarity === 'rare' ? 50 : item.rarity === 'uncommon' ? 25 : 12;
