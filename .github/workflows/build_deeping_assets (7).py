...
def evolve_silhouette(variant, category, companion=False):
    """Turn one of the three hand-built foundations into variants 4+.

    The deformation is performed in world space so a multi-part model changes
    as one silhouette instead of each component merely scaling around its own
    origin. Each treatment changes proportion, taper, lean and/or twist. Small
    category-specific forms are then added to make the new outline readable at
    the compact inventory-view scale.
    """
    convert_curves()
    ...
    half_height = max((max_z - min_z) / 2, .001)

    for obj in meshes:
        inverse = obj.matrix_world.inverted()
        for vertex in obj.data.vertices:
            point = obj.matrix_world @ vertex.co
            t = max(-1.0, min(1.0, (point.z - center_z) / half_height))
            if variant == 4:  # swept / tall
                point.x = point.x * (.82 + .24 * (t + 1) / 2) + .18 * t
                point.y *= .86
                point.z = center_z + (point.z - center_z) * 1.16
            elif variant == 5:  # broad / low
                point.x = point.x * 1.28 + .10 * t
                point.y *= 1.12
                point.z = center_z + (point.z - center_z) * .84
            elif variant == 6:  # helical
                angle = .46 * t
                x, y = point.x, point.y
                point.x = x * math.cos(angle) - y * math.sin(angle)
                point.y = x * math.sin(angle) + y * math.cos(angle)
                point.z = center_z + (point.z - center_z) * 1.05
            elif variant == 7:  # heavy centre / forward lean
                point.x *= .84 + .42 * (1 - abs(t))
                point.y *= .94 + .18 * (1 - abs(t))
                point.x += .24 * t
                point.z = center_z + (point.z - center_z) * .96
            else:  # v8+ uses a rising taper with alternating handedness
                direction = -1 if variant % 2 == 0 else 1
                angle = direction * (.34 + .04 * (variant - 8)) * t
                taper = .78 + .42 * (t + 1) / 2
                x, y = point.x * taper, point.y * (1.08 - .14 * t)
                point.x = x * math.cos(angle) - y * math.sin(angle)
                point.y = x * math.sin(angle) + y * math.cos(angle)
                point.z = center_z + (point.z - center_z) * (1.08 + .015 * (variant - 8))
            vertex.co = inverse @ point

    detail_scale = .82 if companion else 1.0
    if category == "gear":
        if variant == 4:
            for side in (-1, 1):
                cone("SWEPT_FIN", (side * .46, 0, center_z + .32), .16, 0, .62, ACCENT, "ACCENT", 18).rotation_euler[1] = side * .72
        elif variant == 5:
            torus("FORTRESS_BAND", (0, 0, center_z), .72, .085, SECONDARY, "SECONDARY")
            for side in (-1, 1):
                ico("FORTRESS_RIVET", (side * .72, -.06, center_z), (.13, .10, .13), EMISSIVE, "EMISSIVE", 2)
        elif variant == 6:
            for index in range(3):
                angle = math.tau * index / 3
                ico("HELIX_SHARD", (.58 * math.cos(angle), .24 * math.sin(angle), center_z - .35 + index * .35), (.12, .08, .28), EMISSIVE, "EMISSIVE", 2)
        elif variant == 7:
            for side in (-1, 1):
                cone("BRUTAL_SPIKE", (side * .55, 0, center_z + .1), .18, 0, .72, PRIMARY, "PRIMARY", 16).rotation_euler[1] = side * math.pi / 2
        else:
            for index in range(4):
                angle = math.tau * index / 4
                ico("ASCENDANT_GEM", (.62 * math.cos(angle), .25 * math.sin(angle), center_z + .22 * math.sin(angle)), (.14, .09, .2), EMISSIVE, "EMISSIVE", 3)
    else:
        head_z = center_z + half_height * .76
        if variant == 4:
            for side in (-1, 0, 1):
                cone("CROWN_HORN", (side * .3 * detail_scale, 0, head_z), .12 * detail_scale, 0, .58 * detail_scale, ACCENT, "ACCENT", 18).rotation_euler[1] = side * .3
        elif variant == 5:
            for side in (-1, 1):
                cone("SHOULDER_TUSK", (side * .62 * detail_scale, 0, center_z + .2 * detail_scale), .18 * detail_scale, 0, .62 * detail_scale, PRIMARY, "PRIMARY", 18).rotation_euler[1] = side * math.pi / 2
        elif variant == 6:
            for index in range(4):
                cone("DORSAL_SPINE", (0, .18 * detail_scale, center_z - .35 * detail_scale + index * .3 * detail_scale), .11 * detail_scale, 0, .38 * detail_scale, ACCENT, "ACCENT", 16).rotation_euler[0] = math.pi / 2
        elif variant == 7:
            for index in range(3):
                angle = math.tau * index / 3
                ico("ORBIT_SHARD", (.72 * detail_scale * math.cos(angle), .3 * detail_scale * math.sin(angle), center_z + .3 * detail_scale * math.sin(angle)), (.14, .09, .24), EMISSIVE, "EMISSIVE", 2)
        else:
            for side in (-1, 1):
                curve_tube("SWEEP_ANTLER", [(side * .18 * detail_scale, 0, head_z), (side * .44 * detail_scale, .02, head_z + .3 * detail_scale), (side * .7 * detail_scale, .04, head_z + .14 * detail_scale)], .055 * detail_scale, SECONDARY, "SECONDARY")
...
def main():
    for name, builder in GEAR_BUILDERS.items():
        for variant in range(1, VARIANT_COUNT + 1):
            clear_scene()
            foundation = variant if variant <= 3 else 1 + (variant - 4) % 3
            if foundation == 1: builder()
            else: build_gear_variant(name, foundation)
            if variant >= 4: evolve_silhouette(variant, "gear")
            export_asset("gear", name if variant == 1 else f"{name}_v{variant}")
    for archetype in ENEMY_ARCHETYPES:
        for variant in range(1, VARIANT_COUNT + 1):
            clear_scene()
            foundation = variant if variant <= 3 else 1 + (variant - 4) % 3
            if foundation == 1: creature(archetype)
            else: build_creature_variant(archetype, foundation)
            if variant >= 4: evolve_silhouette(variant, "enemy")
    ...
    for form, archetype in PET_FORMS.items():
        for variant in range(1, VARIANT_COUNT + 1):
            ...
            foundation = variant if variant <= 3 else 1 + (variant - 4) % 3
            if foundation == 1: creature(archetype)
            else: build_creature_variant(archetype, foundation, companion=True)
            if variant >= 4: evolve_silhouette(variant, "pet", companion=True)
