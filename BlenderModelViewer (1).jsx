...
export default function BlenderModelViewer({ src, label, size=220, compact=false, profile:providedProfile={}, fallback=null }) {
  ...
  const sourceKey=(Array.isArray(src)?src:[src]).filter(Boolean).join('|');
  ...
  },[sourceKey,compact,profile.primary,profile.secondary,profile.accent,profile.emissive,profile.ghostly,profile.metalness,profile.roughness]);
